...
def index():
    if request.method == 'POST':
                try:
                    ...
                    del image_tags[0]
                    img_data=[]
                    for index,image_tag in enumerate(image_tags):# enumarate will generate the index and content
                                ...
                                with open(os.path.join(save_directory, f"{query}_{image_tags.index(image_tag)}.jpg"), "wb") as f:
                                    f.write(image_data)
                    client = pymongo.MongoClient("mongodb+srv://nethaji257:Kn$9121876724@cluster0.xbjv5fm.mongodb.net/?retryWrites=true&w=majority")
